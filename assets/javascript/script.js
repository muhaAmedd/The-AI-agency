...
function render3D() {
    ...
    // Logo remains stable, solid, and premium (no dynamic wobble or rotation)
...
function initScroll() {
    ...
    if (isMobileView) {
        // �"?�"? MOBILE: No scroll-pinned hero �?" entrance animation handles reveal �"?�"?
        // Do NOT set hero elements visible here; runEntrance() animates them in.
        if (heroLogoEl) gsap.set(heroLogoEl, { z: 0, opacity: 0 }); // logo hidden on mobile
    } else {
    ...
    if (isMobileView) {
        ...
        whoTlMobile.from('.who-content-left .section-title', { y: 40, opacity: 0, duration: 0.8, ease: 'power3.out' })
            .from('.who-main-text', { y: 30, opacity: 0, duration: 0.8, ease: 'power3.out' }, '-=0.5');
    }

    const statNums = document.querySelectorAll('.stat-num');
    ScrollTrigger.create({
        trigger: '.stats-bar', start: 'top bottom', once: true,
        onEnter: () => {
