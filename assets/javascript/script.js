...
function runEntrance() {
    ...
    if (isMob) {
        // ── MOBILE ENTRANCE ──
        // Step 1: Logo appears alone (already visible from loader state)
        // Move logo backwards into the scene
        if (heroLogoEl) {
            tl.to(heroLogoEl, { z: -150, duration: 2.0, ease: 'power3.out' }, '+=0.1');
            tl.call(() => {
                const img = document.getElementById('hero-logo');
                if (img) img.classList.add('dark-shadowed');
            }, null, '-=2.0');
        }
        // Step 2: Header slides down
        tl.to('#site-header', { yPercent: 0, opacity: 1, duration: 0.6, ease: 'power2.out' }, '-=1.6');
        // Step 3: H1 fades in after logo settles
        tl.fromTo('#hero-h1',
            { opacity: 0, y: 30 },
            { opacity: 1, y: 0, duration: 1.2, ease: 'power3.out' },
            '-=0.4'
        );
        // Step 4: Eyebrow, then actions, then stats — staggered sequence
        ...
        // ── DESKTOP ENTRANCE ──
        // Cinematic intro: logo moves back, h1 appears, then sub-elements scroll in
        tl.fromTo('#hero-h1', {opacity:0, scale:0.95, z:-50}, {opacity:1, scale:1, z:0, duration:2.5, ease:'power3.out'}, '+=0.2');
        tl.to('#site-header', { yPercent: 0, opacity: 1, duration: 0.8, ease: 'power2.out' }, '-=2.0');
...
function skipEntrance() {
    ...
    if (heroLogoEl) {
        gsap.set(heroLogoEl, { z: -150, opacity: 1 });
        const img = document.getElementById('hero-logo');
        if (img) img.classList.add('dark-shadowed');
    }
}
...
function initScroll() {
    ...
    // Hero pinned scroll choreography — shorter pin on mobile
    ...
    if (isMob) {
        // On mobile the text is already visible after entrance — scrub a subtle fade
        heroTl.fromTo('.hero-eyebrow', { opacity: 1, y: 0 }, { opacity: 0.6, y: -10, ease: 'power2.in', duration: 1 }, 1.2);
        heroTl.fromTo('.stats-bar',    { opacity: 1, y: 0 }, { opacity: 0.6, y:  10, ease: 'power2.in', duration: 1 }, 1.3);
        heroTl.fromTo('.hero-actions', { opacity: 1, y: 0 }, { opacity: 0.6, y:  10, ease: 'power2.in', duration: 1 }, 1.4);
    } else {
    ...
    function goToReel(idx) {
        ...
        if (isMobile()) {
            // Mobile: show/hide cards by toggling .is-active (button-only, no swipe)
            reelCards.forEach((card, i) => {
                const isActive = i === idx;
                card.classList.toggle('is-active', isActive);
                const vid = card.querySelector('.reel-video-wrap video');
                if (isActive) {
    ...
    ScrollTrigger.create({
        ...
        onLeave: () => {
            ...
            const navArrows = document.querySelector('.reels-nav-arrows');
            if (navArrows && isMobile()) navArrows.classList.remove('is-visible');
        },
        onLeaveBack: () => {
            isGlobalMuted = true;
            allReelVideos.forEach(vid => { vid.muted = true; vid.pause(); });
        ...
        onEnter: () => {
            ...
            const navArrows = document.querySelector('.reels-nav-arrows');
            if (navArrows && isMobile()) navArrows.classList.add('is-visible');
        },
        onEnterBack: () => {
            const activeVideo = reelCards[activeReelIdx].querySelector('.reel-video-wrap video');
            if (activeVideo) {
    ...
    (function() {
        ...
        // Mobile photo overlay arrows (injected once, moved to active slide)
        const mobilePrev = document.createElement('button');
        mobilePrev.className = 'ts-mobile-arrow ts-mobile-prev';
        mobilePrev.setAttribute('aria-label', 'Previous member');
        mobilePrev.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>';

        const mobileNext = document.createElement('button');
        mobileNext.className = 'ts-mobile-arrow ts-mobile-next';
        mobileNext.setAttribute('aria-label', 'Next member');
        mobileNext.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>';

        // Append arrows to the ts-wrap so they float over the photo
        const tsWrap = document.getElementById('ts-wrap');
        if (tsWrap) {
            tsWrap.appendChild(mobilePrev);
            tsWrap.appendChild(mobileNext);
        }

        mobilePrev.addEventListener('click', () => goTo((current - 1 + slides.length) % slides.length));
        mobileNext.addEventListener('click', () => goTo((current + 1) % slides.length));
